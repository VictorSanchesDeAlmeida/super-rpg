...
import { DndClass, DndRace, DndSkill, DndSpell, DndWeapon } from "@/types/dnd";
...
export function CompendiumSheet() {
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-4xl w-[95vw] h-[90vh]">
        <DialogHeader>
          <DialogTitle>Compendio D&D 5e</DialogTitle>
          <DialogDescription>
            Visualize classes, racas, pericias, magias e armas disponiveis com
                  ...
                  {dndClasses.map((characterClass) => (
                    <CompendiumCard key={characterClass.id}>
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        Dado de vida: {characterClass.hitDie} | Atributo
                        principal: {characterClass.primaryAbility.join(", ")}
                      </p>
                    </CompendiumCard>
                  ))}
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {dndWeapons.map((weapon) => (
                    <CompendiumCard key={weapon.id}>
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        Categoria: {weapon.category} | Dano: {weapon.damage} (
                        {weapon.damageType})
                      </p>
                    </CompendiumCard>
                  ))}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {dndSpells.map((spell) => (
                    <CompendiumCard key={spell.id}>
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        Nivel: {spell.level} | Escola: {spell.school}
                      </p>
